...
### 9. Missing Database Indexes on Wave 3 Tables (050-064)
...
- **Fix**: Add migration 065 with indexes on all FK columns and commonly filtered fields.
...
- **Fix**: Add morgan for HTTP logs, structured logging with winston/pino for application logs.

### 11. DB Wrapper Silently Swallows Errors
...
- **Fix**: Only catch specific SQLite `RETURNING` incompatibility errors, rethrow others.
...
### 13. N+1 Query Problem in Delivery Route
...
- **Fix**: Use a single JOIN query to fetch all orders with items.

### 14. Silent Audit Logger Failures
...
- **Fix**: Replace with proper logger. Set log levels per environment.
...
- **Fix**: Handle `SIGTERM`/`SIGINT`, drain connections, close DB, clear intervals.
...
### 33. No Health Check for Database
...
### 35. App.tsx Route Boilerplate (DRY Violation)
- **File**: `client/src/App.tsx` — ~480 lines
...
- **Fix**: Create a route config array and build routes programmatically.

### 36. Incomplete Logout Cleanup
...
- **Fix**: Create a `useLogout()` hook that clears all stores + calls `queryClient.clear()`.

### 37. Theme Flash on Load (FOUC)
...
- **Fix**: Call `useSettingsStore.getState().hydrate()` synchronously before `ReactDOM.createRoot()`.

### 38. Missing Loading States on Mutation Buttons
...
- **Fix**: Disable buttons and show spinner during `mutation.isPending`.
...
### 41. Accessibility — Missing ARIA Labels
...
- **Fix**: Add `aria-label` to all icon buttons. Add `aria-live="polite"` to notification regions.
...
12. DRY up App.tsx route config (#35)
13. Add structured logging (#10, #17)
14. Add missing Wave 3 indexes (#9)
15. Fix N+1 query in delivery (#13)

### Sprint 4 — Client Quality
16. Fix theme flash on load (#37)
17. Add loading states to mutation buttons (#38)
18. Fix logout to clear all stores + cache (#36)
19. Translate Zod validation messages (#40)
20. Add ARIA labels to all icon buttons (#41)

### Sprint 5 — Production Readiness
21. Add graceful shutdown (#21)
22. Add DB health check (#33)
23. Add request logging middleware (#10)
