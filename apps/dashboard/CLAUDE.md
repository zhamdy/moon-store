...
Subsystem contracts for the React SPA. These load when you work under `apps/dashboard/`; the
...
The queue in `localStorage` is replayed by `apps/dashboard/src/shared/hooks/useOffline.ts`. A failed
...
`apps/dashboard/config/pwa.ts`. Two decisions there are load-bearing:
...
Install icons live in `apps/dashboard/public/pwa-{192x192,512x512}.png`, generated from
