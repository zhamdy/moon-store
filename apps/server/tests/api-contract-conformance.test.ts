...
describe('API contract conformance guardrails', () => {
  it('keeps legacy success flags out of module controllers', () => {
    const modulesRoot = resolve(root, 'apps/server/src/modules');
    const controllers = readdirSync(modulesRoot, { recursive: true })
      ...
      .concat(
        readdirSync(resolve(root, 'apps/server/middleware'))
          .filter((path) => path.endsWith('.ts'))
          .map((path) => readFileSync(resolve(root, 'apps/server/middleware', path), 'utf8'))
      )
      .join('\n');
  ...
  it('keeps the React notification collection off legacy limit queries', () => {
    const notifications = read('apps/dashboard/src/app/NotificationCenter.tsx');
  ...
  it('keeps migrated React collections off oversized and legacy list requests', () => {
    const clientRoot = resolve(root, 'apps/dashboard/src');
  ...
  it('requires every mounted route group to have an explicit contract manifest entry', () => {
    const routerSource = read('apps/server/src/router.ts');
    const manifestSource = read('apps/server/src/http/endpointManifest.ts');
