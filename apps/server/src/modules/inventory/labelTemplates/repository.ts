...
import { LabelTemplateRecord, CreateLabelTemplateDTO, UpdateLabelTemplateDTO } from './types';
...
export class LabelTemplatesRepository implements ILabelTemplatesRepository {
  ...
  async clearDefault(excludeId?: number | string, queryable?: Queryable): Promise<void> {
    if (excludeId !== undefined) {
      await this.q(queryable).query('UPDATE label_templates SET is_default = 0 WHERE id != $1', [
        excludeId,
      ]);
    } else {
  ...
  async create(data: CreateLabelTemplateDTO, queryable?: Queryable): Promise<LabelTemplateRecord> {
    const res = await this.q(queryable).query<LabelTemplateRecord>(
      ...
      [data.name, data.width_mm, data.height_mm, data.layout_json, data.is_default ? 1 : 0]
    );
    return res.rows[0];
  }
  ...
  async update(
    ...
    const res = await this.q(queryable).query<LabelTemplateRecord>(
      ...
      [data.name, data.width_mm, data.height_mm, data.layout_json, data.is_default ? 1 : 0, id]
    );
    return res.rows[0] || null;
  }
