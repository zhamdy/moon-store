...
export async function fetchSeededFixtures(): Promise<SeededFixtures> {
  const [userRes, catRes, prodRes, custRes, branchRes, distRes, shipRes, coupRes, giftRes] =
    await Promise.all([
      query<{ id: number; email: string }>('SELECT id, email FROM users ORDER BY id ASC LIMIT 1'),
      query<{ id: number; name: string }>(
        'SELECT id, name FROM categories ORDER BY id ASC LIMIT 1'
      ),
      ...
      query<{ id: number; code: string }>(
        'SELECT id, code FROM gift_cards ORDER BY id ASC LIMIT 1'
      ),
    ]);
