  ...
  async (_req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await db.query(
        `SELECT
          c.id, c.name, c.phone,
          (CURRENT_DATE - MAX(s.created_at)::date)::int as recency_days,
      ...
      const customers = result.rows.map((c: any) => {
        const recency =
          c.recency_days === null || c.recency_days === undefined ? 999 : Number(c.recency_days);
        const frequency = Number(c.frequency || 0);
        const monetary = Number(c.monetary || 0);
      ...
      const summary = segments.map((seg) => {
        ...
        return {
          ...
          total_revenue: segCustomers.reduce(
            (sum: number, c: any) => sum + (Number(c.monetary) || 0),
            0
          ),
          avg_frequency:
            segCustomers.length > 0
              ? segCustomers.reduce((sum: number, c: any) => sum + (Number(c.frequency) || 0), 0) /
                segCustomers.length
              : 0,
        };
      });
