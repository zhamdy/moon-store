...
router.get('/dashboard', verifyToken, requireRole('Admin'), async (req, res, next) => {
  try {
...
router.get('/top-products', verifyToken, requireRole('Admin'), async (req, res, next) => {
  try {
    const { from, to } = req.query;
...
router.get('/payment-methods', verifyToken, requireRole('Admin'), async (req, res, next) => {
  try {
    const { from, to } = req.query;
    let dateFilter = "created_at >= date('now', '-30 days')";
    let params = [];
...
router.get('/orders-per-day', verifyToken, requireRole('Admin'), async (req, res, next) => {
  try {
    const { from, to } = req.query;
    let dateFilter = "created_at >= date('now', '-30 days')";
    let params = [];
