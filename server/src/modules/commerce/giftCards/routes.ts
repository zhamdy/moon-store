...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const balanceData = await getGiftCardBalance(code);
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      let result;
      try {
        result = await redeemGiftCard(code, amount, sale_id, authReq.user!.id);
      } catch (err: any) {
        if (
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { card, transactions } = await getGiftCardTransactions(id);
...
router.put(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const updated = await updateGiftCardStatus(id, parsed.data.status);
