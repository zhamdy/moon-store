...
function MonthGrid({ month, selected, onDayClick, showOutsideDays }) {
  ...
  const isRangeStart = (day) => selected?.from && isSameDay(day, selected.from);
  const isRangeEnd = (day) => selected?.to && isSameDay(day, selected.to);
  const isSelected = (day) => isRangeStart(day) || isRangeEnd(day);
...
function Calendar({
  ...
  const [baseMonth, setBaseMonth] = useState(() => selected?.from || new Date());
