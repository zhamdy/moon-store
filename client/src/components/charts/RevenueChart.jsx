...
const CustomTooltip = ({ active, payload, label, isDark }) => {
  ...
  return (
    ...
    >
      <p className="text-xs font-data" style={{ color: isDark ? '#6B6B6B' : '#888888' }}>{format(new Date(label), 'MMM dd, yyyy')}</p>
      <p className="text-sm font-semibold text-gold font-data">{formatCurrency(payload[0].value)}</p>
    </div>
  );
};
